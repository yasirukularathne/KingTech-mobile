...
export function PointerHighlight() {
	...
	useEffect(() => {
		...
		const update = () => {
			...
			if (label) label.style.transform = `translate3d(${rx}px, ${ry - 18}px, 0)`;
		...
		const setHoverState = (el: Element | null) => {
			...
			let text = "";
			const isInteractive = !!el && (el.closest("a,button,[role=button]") != null);
			const dataEl = el?.closest("[data-cursor-text]") as HTMLElement | null;
			if (dataEl?.dataset.cursorText) text = dataEl.dataset.cursorText;
			else if (isInteractive) text = (el as HTMLElement).getAttribute("aria-label") || (el as HTMLElement).innerText?.trim().slice(0, 16) || "Open";
		...
		const spawnRipple = (x: number, y: number) => {
			...
			ripple.style.transition = "transform 500ms cubic-bezier(.2,.8,.2,1), opacity 500ms ease";
